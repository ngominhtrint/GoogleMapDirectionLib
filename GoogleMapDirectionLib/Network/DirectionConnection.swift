...
public final class DirectionConnection: DirectionService {
  ...
  public func getDirection(origin: String, destination: String, waypoints: String, transportMode: String, departureTime: String, language: String, units: String, avoid: String, transitMode: String, alternatives: Bool, apiKey: String) -> Single<Direction> {
  
    let directionMapper = Mapper<Direction>()
    let directionEndPoint = buildUrl(origin: origin, destination: destination, waypoints: waypoints, transportMode: transportMode, departureTime: departureTime, language: language, units: units, avoid: avoid, transitMode: transitMode, alternatives: alternatives, apiKey: apiKey)
    
    return RxAlamofire.request(.get, directionEndPoint, parameters: nil, encoding: JSONEncoding.default, headers: nil)
      .debug()
      .responseJSON()
      .observeOn(scheduler)
      .debug()
...
extension DirectionConnection {
  ...
  func buildUrl(origin: String, destination: String, waypoints: String, transportMode: String, departureTime: String, language: String, units: String, avoid: String, transitMode: String, alternatives: Bool, apiKey: String) -> String {
